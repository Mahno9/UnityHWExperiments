...
namespace MiniGame
{
    public class LevelBuilder : MonoBehaviour
    {
        ...
        public void BuildLevelBox(int width, int height)
        {
            ...
            {
                float currentX = x;
                if (currentX + wallLength > totalWidth)
                {
                    currentX = Mathf.Max(0, totalWidth - wallLength);
                }
            ...
            {
                float currentZ = z;
                if (currentZ + wallLength > totalHeight)
                {
                    currentZ = Mathf.Max(0, totalHeight - wallLength);
                }
